...
On load, common scale_* functions are overwritten with scale_fill_nycc() and scale_color_nycc().
...
Additionally, `mapshot()` overwrites `mapview::mapshot()` to force map saves as pdf for increased picture quality and warns users if they choose a different file type. Use `mapview::mapshot` if you want the original mapshot function. 
