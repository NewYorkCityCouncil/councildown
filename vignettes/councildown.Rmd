...
council_geo <- get_geo_estimates("council") %>%
  select(council, percent_work_from_home_workers_16_and_older, geometry)
```
...
map <- leaflet() %>%
  ...
  addPolygons(data = council_geo,
              ...
              label = ~lapply(councilPopup(
                paste0("<h3>", paste0("CD: ",council), "</h3>",
...
htmlwidgets::saveWidget(map, file="visuals/{map_html}", selfcontained = T)
```
