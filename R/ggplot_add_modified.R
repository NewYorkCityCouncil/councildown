...
ggplot_add.Scale <- function(object, plot, object_name) {
  if (!inherits(plot$facet, "FacetNull")) {
    cli::cli_abort("Can't add {.var {object_name}} to a {.cls ggplot} object when there are facets.")
  }
...
ggplot_add.Facet <- function(object, plot, object_name) {
  if( length(plot$scales$scales) != 0){
    cli::cli_abort("Can't add {.var {object_name}} to a {.cls ggplot} object when there are scales.")
  }
  plot$facet <- object
  plot
}
...
ggplot_add.Layer <- function(object, plot, object_name) {
  ...
  plot$labels <- ggplot2:::defaults(current_labels[!current_fallbacks], new_labels)
  if (any(current_fallbacks)) {
    plot$labels <- ggplot2:::defaults(plot$labels, current_labels)
  }
  plot
}